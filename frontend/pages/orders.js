...
export default function MyOrdersPage() {
  ...
  if (!user && !loading) {
    return <div className="container"><h1>My orders</h1><p>You need to be signed in to view your orders.</p><Link className="btn" href="/login">Sign in</Link></div>;
  }
  if (loading) return <div className="container"><p>Loading orders...</p></div>;
  if (error) return <div className="container"><p style={{ color: "red" }}>{error}</p></div>;
  if (empty) return <div className="container"><h1>My orders</h1><p>You have no orders yet.</p><Link className="btn" href="/">Browse products</Link></div>;
          ...
          return (
            <div key={order.id} style={{ border: "1px solid var(--border)", borderRadius: 12, padding: 14, background: "rgba(255,255,255,0.95)" }}>
              ...
              <div style={{ marginTop: 10, display: "grid", gap: 4 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between" }}><span>Shipping</span><strong>{shipping === 0 ? "Free" : formatRON(shipping)}</strong></div>
              </div>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                <Link className="btn" href={`/order-success?orderId=${encodeURIComponent(order.id)}`}>View details</Link>
                {status === "pending" && (
                  <button
                    className="btn"
